...
@pytest.fixture
def generated_project(tmp_path: Path, meta_visibility: str, execution_mode: str) -> Path:
...
class TestGitHubWorkflows:
    ...
    def test_check_commit_authors_absent_by_default(self, generated_project: Path) -> None:
...
class TestClaudeSettings:
    ...
    def test_safe_mode_has_ask(self, generated_project: Path, execution_mode: str) -> None:
        if execution_mode != "safe":
            pytest.skip("only for safe mode")
        settings = json.loads((generated_project / ".claude" / "settings.json").read_text())
        assert len(settings["permissions"].get("ask", [])) > 0

    def test_full_auto_has_empty_ask(self, generated_project: Path, execution_mode: str) -> None:
...
class TestMetaVisibility:
    ...
    def test_public_meta_not_ignored(self, generated_project: Path, meta_visibility: str) -> None:
    ...
    def test_private_meta_ignored(self, generated_project: Path, meta_visibility: str) -> None:
...
class TestSkills:
    ...
    def test_research_has_output_schema(self, generated_project: Path) -> None:
        schema = generated_project / ".claude" / "skills" / "research" / "output-schema.md"
...
class TestTechWatchYAGNI:
    ...
    def test_research_themes_absent_at_generation(self, generated_project: Path) -> None:
    ...
    def test_tech_watch_sweep_package_present(self, generated_project: Path) -> None:
        assert (generated_project / "scripts" / "tech_watch" / "sweep" / "core.py").is_file()
    ...
    def test_tech_watch_optional_dep_declared(self, generated_project: Path) -> None:
        ...
        assert "tech-watch" in optional_deps, "tech-watch optional dep group must be declared"
...
class TestPreCommitConfig:
    ...
    def test_valid_yaml(self, generated_project: Path) -> None:
        config = yaml.safe_load((generated_project / ".pre-commit-config.yaml").read_text())
    ...
    def test_skills_contract_passes_on_fresh_project(self, generated_project: Path) -> None:
...
class TestMetaParity:
    ...
    def test_meta_skills_mirror_template(self) -> None:
        tpl = {p.name for p in (ROOT / "template" / ".claude" / "skills").iterdir() if p.is_dir()}
    ...
    def test_meta_agents_mirror_template(self) -> None:
        ...
        meta = {p.stem for p in (ROOT / ".claude" / "agents").iterdir() if p.suffix == ".md"}
        assert tpl == meta, f"agent parity mismatch: tpl={tpl} meta={meta}"

    def test_meta_rules_mirror_template(self) -> None:
        tpl = {
            p.name for p in (ROOT / "template" / ".claude" / "rules").iterdir() if p.suffix == ".md"
        }
        meta = {p.name for p in (ROOT / ".claude" / "rules").iterdir() if p.suffix == ".md"}
